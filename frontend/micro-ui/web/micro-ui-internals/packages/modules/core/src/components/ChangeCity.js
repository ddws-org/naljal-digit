  ...
  useEffect(() => {
    ...
    unique?.forEach((uniCode) => {
      data?.MdmsRes?.["tenant"]["tenants"]?.map((items) => {
        if (items?.code !== "mp" && items?.code === uniCode) {
          filteredArray.push({
            label: `${prop?.t(Digit.Utils.locale.convertToLocale(items?.blockcode, "EGOV_LOCATION_DIVISION"))} - ${prop?.t(
              `TENANT_TENANTS_${stringReplaceAll(uniCode, ".", "_")?.toUpperCase()}`
            )}`,
            value: uniCode,
          });
        } else if (items?.code === "mp" && items?.code === uniCode) {
          filteredArray.push({
