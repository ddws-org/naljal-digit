...
const PermissionCheck = ({ permissions, t }) => {
  const [approvalChecks, setApprovalChecks, clearApprovals] = Digit.Hooks.useSessionStorage(
    "OBPS_APPROVAL_CHECKS",
    permissions?.map((permission) => ({ label: permission, checked: false }))
  ); //useState(() => permissions?.map(permission => ({ label: permission, checked: false })))
  ...
  return (
    <div>
      ...
      {newApprovals?.map((approval, index) => (
        <div>
          ...
          <TextInput
            ...
            textInputStyle={{ maxWidth: "830px", width: "830px" }}
      ...
      <LinkButton
        style={{ color: "#1f4ac4", maxWidth: "10%", /* float: "right", marginTop: "-50px", */ marginRight: "3%" }}
