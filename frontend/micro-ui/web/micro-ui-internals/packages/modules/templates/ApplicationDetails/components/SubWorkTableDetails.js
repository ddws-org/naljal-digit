...
const SubWorkTableDetails = ({ data }) => {
  ...
  const renderHeader = (headers) => {
    return headers?.map((key, index) => {
      return (
        <th key={index} style={getStyles(key)}>
          {" "}
          {t(key)}{" "}
        </th>
      );
        ...
        <tr style={{ height: "50%" }}>
          ...
          <td style={getStyles(2)}>
            ...
            {row[1] === t("WORKS_TOTAL_AMT") ? (
              <div style={{ float: "right", fontWeight: "bold" }}>{row[1]}</div>
            ) : (
          ...
          {row[1] === t("WORKS_TOTAL_AMT") ? (
            <td style={{ width: "15vw" }}>
              <div style={{ float: "right", fontWeight: "bold" }}>{row[2]}</div>
            </td>
          ) : (
            <td style={{ width: "15vw" }}>
              <div style={{ float: "right" }}>{row[2]}</div>
            </td>
          )}
          {row[3] && (
            <td style={getStyles(3)}>
              <div
                style={{ display: "flex", flexDirection: "row", cursor: "pointer", color: "#1f4ac4" }}
                onClick={() =>
                  history.push({
    ...
    <table className="table reports-table sub-work-table">
      ...
      <tbody>
        ...
        {/* <tr>
                  ...
                  <td style={{ ...getStyles(2), "textAlign": "center" }} onClick={addRow}><span><AddIcon fill={"#1f4ac4"} styles={{ "margin": "auto", "display": "inline", "marginTop": "-2px" }} /><label style={{ "marginLeft": "10px" }}>{t("WORKS_ADD_ITEM")}</label></span></td>
