      ...
      <div className="complaint-links-container">
        ...
        <div className="body" style={{ margin: "0px", padding: "0px" }}>
          ...
          <div className="links-wrapper" style={{ width: "100%" }}>
            {links
              .reduce(
                (acc, { count, label, link, category }, index) => {
                ...
                { currentCategory: null, links: [] }
              ) // Initial accumulator state
              .links.filter(({ items }) => items.length > 0) // Filter out categories with no items
              .map(({ category, items }, index) => (
                <>
                  {category && <div className="category-header">{category // Display category header if category exists
                    }</div>}
                  <div className="category-item">
                    {items.map((item, subIndex) => (
                      <div className="link-row" key={`${index}-${subIndex}`}>
                        ...
                        {item.count ? (
                          <>
                            {FsmHideCount ? null : <span className={"inbox-total"}>{item.count || "-"}</span>}
                            <Link to={item.link}>
                              <ArrowRightInbox />
                            </Link>
                          </>
