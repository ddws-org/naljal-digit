...
iphard= '{"mobileNumber":"9980770587","messagAllUserMerged_12092022e":"प्रिय  Mani, आपको पंजाब के mGramSeva एप्लिकेशन में आमंत्रित किया गया है। कृपया https://mgramseva-qa.egov.org.in/mgramseva/login का उपयोग करके लॉगिन करें। उपयोगकर्ता नाम: 9980770587 पासवर्ड: eGov@123. EGOVS","category":"TRANSACTION","expiryTime":null}'
def connect():
...
def createUser(sheet, rowIndex, accesstoken,file):
    ...
    father_name=str(sheet.cell(rowIndex, 5).value).upper()
    if(father_name=='NONE'):
        father_name='NA'
    user['fatherOrHusbandName'] = father_name
    ...
    if (userSearchResponse==None or  len(userSearchResponse) == 0):
          ...
          if( jsondata.get('user') != None ):
                ...
                sheet.cell(rowIndex, 14).value = 'Success'
            ...
            sheet.cell(rowIndex, 14).value = 'Failure'
            error = jsondata['Errors'][0]['code'] + ' ' + jsondata['Errors'][0]['message']
            sheet.cell(rowIndex, 13).value = error
    else:
          print("updating user...")
          ...
          if( jsondata.get('user') != None ):
                sheet.cell(rowIndex, 13).value = jsondata.get('user')[0]['id']
                sheet.cell(rowIndex, 14).value = 'Success(Updated)'
          else:
              sheet.cell(rowIndex, 14).value = 'Failure(updated)'
              error = jsondata['Errors'][0]['code'] + ' ' + jsondata['Errors'][0]['message']
              sheet.cell(rowIndex, 13).value = error
