...
def update_entity(idd,name, code, level, child_list, newchild):
    ...
    update_status = name + ' updated at level ' + level
...
def search_api(name, code, level, search_result):
    ...
    if departmentEntity:
        ...
        if level < 6:
            update_entity(cid,name, code, level, childsearch, childid)
            
    if cid is None or cid == '':
        cid, child_list = create_entity(name, code, level)

        cid_status = name + ' at level '+level+ ' created'
        if level < 6:
        ...
        if(name!=None):
            cid_status = name + ' level '+ level+' updated'
            update_stat = None
