...
//    private static String holderSelectValues = "{HOLDERSELECTVALUES}";
	...
	private static final String WATER_SEARCH_QUERY = "SELECT count(*) OVER() AS full_count, conn.*, wc.*, document.*, plumber.*, wc.connectionCategory, wc.connectionType, wc.waterSource,"
			...
			+ " conn.id as conn_id, conn.tenantid, conn.applicationNo, conn.ihlDetail as ihlDetail,  conn.sbmAccountno as sbmAccountno, conn.applicationStatus, conn.status, conn.connectionNo, conn.oldConnectionNo, conn.property_id, conn.roadcuttingarea,"
